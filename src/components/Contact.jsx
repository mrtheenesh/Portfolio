...
/* small reusable variant for staggered items */
const slideUp = {
  hidden: { opacity: 0, y: 30 },
  visible: (i = 0) => ({
    opacity: 1,
    y: 0,
    transition: { delay: 0.08 * i + 0.06, duration: 0.6, ease: [0.25, 0.1, 0.25, 1] },
  }),
};
...
export default function Contact() {
  ...
  const inView = useInView(sectionRef, { amount: 0.35 }); // triggers when ~35% visible
  const scrollDir = useScrollDirection();

  // Play only when we enter AND scrolling down. Reset to hidden when leaving so downward entry replays.
  useEffect(() => {
    ...
    // if inView && scrollDir === "up" => do nothing (keeps current state — no animation)
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_y22suor", // service ID
        "template_2al08u5", // template ID
        formRef.current, // form ref
        "G_qfdI4PxvrJU_5L5" // public key
      )
      .then(
        () => {
    ...
    >
      ...
      >
        ...
        <motion.div initial="hidden" animate={controls} variants={slideUp} custom={4}>
          <button type="submit" className="w-full py-3 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 transition">
            Send Message
          </button>
        </motion.div>
      </motion.form>
